...
class FrameTimer {
public:
	std::chrono::duration<std::chrono::nanoseconds> max_frame_duration_in_nanoseconds;
	...
	constexpr FrameTimer(uint16_t max_FPS) noexcept {
		...
		max_frame_duration_in_nanoseconds = (std::chrono::duration<std::chrono::nanoseconds>)(std::chrono::nanoseconds)(nanoseconds_per_second / max_FPS);
	}

	void mark_frame_start() noexcept { frame_start = std::chrono::high_resolution_clock::now(); }

	void wait_if_needed() const noexcept {
		std::chrono::duration<std::chrono::nanoseconds> frame_duration = (std::chrono::duration<std::chrono::nanoseconds>)(std::chrono::high_resolution_clock::now() - frame_start);
